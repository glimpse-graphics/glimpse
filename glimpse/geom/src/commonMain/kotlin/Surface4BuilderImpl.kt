...
internal class Surface4BuilderImpl<T>(
    ...
    override fun ofType(curveType: CurveType): Surface4.Builder<T> {
    ...
    override fun build(): Surface4<T> {
        if (curveType != CurveType.B_SPLINE && (knotsU + knotsV).isNotEmpty()) {
        ...
        return Surface4Impl(
            curveType = requireNotNull(this.curveType) { "Surface type must be specified" },
            degree = this.degree,
