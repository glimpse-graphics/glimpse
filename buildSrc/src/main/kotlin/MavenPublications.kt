...
fun PublicationContainer.multiplatformPublications(project: Project) {
    filterIsInstance<MavenPublication>().forEach { publication ->
        publication.artifactId = "${project.parent?.name}-${publication.artifactId}"
        publication.artifact(project.tasks["javadocJar"])
        publication.pom { commonPom(project) }
    }
}

fun PublicationContainer.registerJarPublication(project: Project) {
